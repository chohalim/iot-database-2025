...
-- GROUP BY 사용할 떈 반드시 집계함수 및 GROUP BY에 포함된 컬럼이 SELECT 안에 포함시켜야 함
SELECT custid
	 , COUNT(custid) AS 구매도서수량  -- 함수 매개변수로 *, custid
...
SELECT custid
 ...
 GROUP BY custid
HAVING 총수량 >= 2; -- COUNT(*)와 별명 사용할 수 있음
...
SELECT name, SUM(saleprice)
 ...
 ORDER BY Customer.nameCustomer;
...
-- 3-27 도서를 구매하지 않은 고객을 포함해서 고객의 이름과 고객이 주문한 도서의 판매가격을 구하시오.
...
SELECT C.name
  FROM Customer AS C
 WHERE C.custid IN( -- IN을 써야됨 =는 안됨
...
-- EXISIS : 상관서브쿼리에서 사용하는 키워드, 상관서브커리 아니라도 사용가능
